...
export default function TeamModal({ team, onClose }: TeamModalProps) {
  ...
  const createTeamMutation = useMutation({
    mutationFn: async (data: any) => {
      const response = await apiRequest('POST', '/api/teams', {
        name: data.name,
        captainDiscordId: "admin-created",
  ...
  const updateTeamMutation = useMutation({
    mutationFn: async (data: any) => {
      ...
      for (const member of data.members) {
        if (member.id) {
          // Existing member - find and update if needed
      ...
      const updatedMemberIds = data.members.map((m: any) => m.id).filter(Boolean);
      const membersToDelete = currentMembers.filter(
        m => !m.isCaptain && !updatedMemberIds.includes(m.id)
      );
  ...
  const handleSubmit = () => {
    if (!formData.name || !formData.captainUsername || !formData.captainInGameId) {
      toast({
    ...
    for (const member of formData.members) {
      if (!member.username || !member.inGameId) {
        toast({
          title: "Validation error",
          description: "All team members must have a username and in-game ID.",
          variant: "destructive",
        });
        return;
      }
